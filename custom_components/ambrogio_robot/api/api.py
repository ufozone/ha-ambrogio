...
class AmbrogioRobotApiClient:
    ...
    async def async_check_api_connect(self) -> bool:
        ...
        result = await self.execute("thing.find", {"key": self._access_token})
        if result is False:
            return False
    ...
    async def async_check_robot(self, robot_imei: str) -> bool:
        ...
        return await self.execute("thing.find", {"imei": robot_imei})
        ...
        try:
            async with async_timeout.timeout(60):
                ...
                if not response.status == 200:
                    raise AmbrogioRobotApiClientError("Failed to POST to API")
    ...
    # @return    bool      Success or failure to authenticate.
    async def app_auth(
        self, api_key: str, access_token: str, update_session_id: bool = True
    ) -> bool:
        """Authenticate the application."""
        try:
            params = {
